...
def refund(self, player: arc4.Address) -> None:
    # Available after 172800 rounds (~48h) since committed_round with no activity
    # Permissionless — any wallet can trigger
    # Returns bet + MBR to player, deletes box
```
...
if keeper fails 48h: player calls refund() permissionlessly
```
